...
├── manifest.json               # PWA 配置（图标指向根目录 icon-192.png / icon-512.png）
├── sw.js                       # Service Worker（离线缓存）
├── icon-192.png                # PWA 图标（192）
├── icon-512.png                # PWA 图标（512）
├── cloud-functions/            # 三个云函数（见各自 README，按需部署）
...
3. 想让它「可安装到手机桌面 / 离线可用」：把它和同目录的 `manifest.json`、`sw.js`、`icon-192.png`、`icon-512.png` 一起部署到任意静态托管（见下）

## 部署到云端（CloudBase）

### 1. 静态托管（前端）

把 `LunarShift-prototype.html`、`manifest.json`、`sw.js`、`icon-192.png`、`icon-512.png` 上传到 CloudBase 静态托管（或 GitHub Pages / Vercel / Netlify 任意静态服务）即可。
...
### Run
Just open `LunarShift-prototype.html`. For installable/offline PWA, host it together with `manifest.json`, `sw.js`, `icon-192.png`, `icon-512.png` on any static host.
